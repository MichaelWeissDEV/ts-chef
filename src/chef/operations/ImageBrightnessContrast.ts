/**
 * @fileoverview ImageBrightnessContrast operation - Ported from GCHQ's CyberChef
 ...
 */
export class ImageBrightnessContrast extends TypedOperation<ArrayBuffer, Promise<AnyInput>, unknown[]> {
  /**
  ...
  async run(input: ArrayBuffer, args: unknown[]): Promise<AnyInput> {
    const [brightness, contrast] = args as [number, number];
    if (!isImage(input)) {
