...
describe("DefangURL", () => {
  ...
  test("Defangs http:// to hxxp://", () => {
    const result = op.run("http://example.com", [true, true, true, "Valid domains and full URLs"]);
    expect(result).toContain("hxxp");
  });

  test("Replaces dots with [.]", () => {
    const result = op.run("example.com", [true, true, true, "Valid domains and full URLs"]);
  ...
  test("Plain text without URL is unchanged", () => {
    const result = op.run("hello world", [true, true, true, "Valid domains and full URLs"]);
